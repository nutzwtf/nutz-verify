...
| `transfers` | The whole NUTZ `Transfer(from, to, value)` history **from the token's creation block**, not just this Epoch's. Entries before the window establish opening balances; a history that does not reach back far enough drives a balance negative and is rejected rather than computed from. `from`/`to` of the zero address are the mint and burn counterparty, which is never a Holder. **Order is not significant, including within a single block**: balances are replayed in timestamp order and judged only once every transfer sharing a timestamp has been applied, so a wallet that receives and forwards in the same block is not mistaken for a broken history in whichever order the logs arrive. |

...
- The Cache needs its own correctness work rather than inheriting a store's: fixed 124-byte stride (`[120-byte payload][4-byte CRC32C]`), truncate-to-last-valid-record on open, batched fsync (per-record costs ~6.7×), and a fixed file header carrying magic, format version, chain id, contract address and the start block the history was read from, so a file from the wrong chain — or a history that began later than this run assumes — is rejected loudly. No sidecar manifest: the CRC chain plus the per-record block number make the log self-describing. Durability requirements are unusually weak — the log is a cache of public data, so we must *detect* damage, not survive it.
